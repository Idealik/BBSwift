...
class UserFirebase {
    ...
    func getUserFromSnapshot(userSnapshot: DataSnapshot)-> User {
        ...
        user.surname = userSnapshot.childSnapshot(forPath: User.SURNAME).value as! String
        user.city = userSnapshot.childSnapshot(forPath: User.CITY).value as! String
        user.phone = userSnapshot.childSnapshot(forPath: User.PHONE).value as! String
        user.rating = userSnapshot.childSnapshot(forPath: User.AVG_RATING).value as! Float
        user.photoLink = userSnapshot.childSnapshot(forPath: User.PHOTO_LINK).value as! String
        user.subscribersCount = userSnapshot.childSnapshot(forPath: User.COUNT_OF_SUBSCRIBERS).value as! Int
        user.subscriptionsCount = userSnapshot.childSnapshot(forPath: User.COUNT_OF_SUBSCRIPTIONS).value as! Int
        user.countOfRates = userSnapshot.childSnapshot(forPath: User.COUNT_OF_RATES).value as! Int
